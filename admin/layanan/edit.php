...
// Cek apakah ID valid
if ($id <= 0) {
    setFlashMessage('error', 'ID layanan tidak valid');
    header('Location: index.php');
    exit;
}

// Ambil data layanan
$layanan = querySingle($conn, "SELECT * FROM layanan WHERE id = $id");
...
// Inisialisasi variabel dengan data layanan
$judul = $layanan['judul'];
$slug = $layanan['slug'];
$deskripsi_singkat = $layanan['deskripsi_singkat'];
$deskripsi = $layanan['deskripsi'];
$urutan = $layanan['urutan'];
$status = $layanan['status'];
$icon = $layanan['icon'];
$gambar_lama = $layanan['gambar'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($judul)) {
        ...
        // Cek apakah slug sudah ada (kecuali untuk layanan ini sendiri)
        $cekSlug = querySingle($conn, "SELECT id FROM layanan WHERE slug = '$slug' AND id != $id");
        ...
        if ($cekSlug) {
            ...
            if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] == 0) {
                ...
                if ($uploadResult['status'] == 'success') {
                    ...
                    if (!empty($gambar_lama)) {
                        $gambarPath = $uploadDir . $gambar_lama;
            ...
            if (empty($error)) {
                ...
                if (update($conn, 'layanan', $id, $data)) {
                    // Log aktivitas
                    $userId = $currentUser['id'];
                    $conn->query("INSERT INTO log_aktivitas (user_id, tipe, deskripsi) VALUES ($userId, 'update', 'Memperbarui layanan: $judul')");
...
<head>
    ...
    <title>Edit Layanan - Puskesmas Kepulauan Seribu Selatan</title>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="mt-2">
                                            <?php if (!empty($gambar_lama)): ?>
                                                <img id="gambar-preview" src="../../uploads/layanan/<?php echo $gambar_lama; ?>" alt="Preview" class="img-thumbnail" style="max-width: 200px; max-height: 200px;">
                                            <?php else: ?>
